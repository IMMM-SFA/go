...
def data_reader(ba_file, load_file, wecc_file, states_shapefile, init_epsg, target_epsg):

    df_BAs = pd.read_csv(ba_file, header=0)
    ...
    filter_nodes = gpd.GeoDataFrame(df, crs=f'epsg:{init_epsg}', geometry=geometry)
    nodes_df = gpd.GeoDataFrame(df, crs=f'epsg:{init_epsg}', geometry=geometry)
    nodes_df = nodes_df.to_crs(epsg=target_epsg)

    BAs_gdf = gpd.read_file(wecc_file)
    BAs_gdf = BAs_gdf.to_crs(epsg=target_epsg)

    states_gdf = gpd.read_file(states_shapefile)
    states_gdf = states_gdf.to_crs(epsg=target_epsg)
    ...
    return B, BAs, filter_nodes, joined


def remove_redundant_buses(joined, BAs, B, target_epsg):
    """Eliminate redundant buses (overlapping BAs) based on peak load and location within BAs"""
    ...
    for b in B:
        ...
        if b_idx < 1:
            ...
            combined = gpd.GeoDataFrame(selection,
                                        crs=target_epsg,
                                        geometry="geometry")
...
def remove_non_desired_buses(combined, BAs, nodes_to_ba_state_file):
    ...
    combined = combined.reset_index(drop=True)
    combined.to_csv(nodes_to_ba_state_file)

    return combined


def read_in_generators(nodes_to_ba_state_file, generators_file):

    df_BA_states = pd.read_csv(nodes_to_ba_state_file,index_col=0)
    df_gens = pd.read_csv(generators_file)

    return df_BA_states, df_gens


def remove_non_tell(df_BA_states, df_gens):
    ...
    for n in unique_bus_names:
        ...
        for i in sample_ba:
            if i in s:
                ...
                s_bn = list(itertools.compress(sample_bus_number,idx))
                s_cp = list(itertools.compress(sample_bus_cap,idx))
                mx = np.max(s_cp)
                total = np.sum(s_cp)
                idx2 = s_cp.index(mx)
    ...
    return nodes, reduced_gen_buses, df_BA_states, caps


def load(load_file, nodes, reduced_gen_buses, df_BA_states, caps):
    ...
    return load_weights, gen_weights, states, BAs, keys, max_loads, non_zero, df_load, reduced_gen_buses


def nodal_reduction(RTS, keys, max_loads, non_zero, df_load, filter_nodes, distance_threshold,
    ...
    for NN in RTS:
        ...
        f = os.path.join(node_output_dir, 'excluded_nodes_' + str(NN) + '.csv')
        ...
        f = os.path.join(node_output_dir, 'selected_nodes_' + str(NN) + '.csv')
        df_selected_nodes.to_csv(f, index=None)


def mapper(ba_file: str,
           load_file: str,
           wecc_file: str,
           states_shapefile: str,
           nodes_to_ba_state_file: str,
           generators_file: str,
           node_output_dir: str,
           rts_list: List[int],
           init_epsg: int = 4326,
           target_epsg: int = 2163,
           distance_threshold: int = 5):
    """Workhorse function for mapper."""

    B, BAs, filter_nodes, joined = data_reader(ba_file, load_file, wecc_file, states_shapefile, init_epsg, target_epsg)

    combined = remove_redundant_buses(joined, BAs, B, target_epsg)

    combined = remove_non_desired_buses(combined, BAs, nodes_to_ba_state_file)

    df_BA_states, df_gens = read_in_generators(nodes_to_ba_state_file, generators_file)

    nodes, reduced_gen_buses, df_BA_states, caps = remove_non_tell(df_BA_states, df_gens)

    load_weights, gen_weights, states, BAs, keys, max_loads, non_zero, df_load, reduced_gen_buses = load(load_file,
                                                                                                         nodes,
                                                                                                         reduced_gen_buses,
                                                                                                         df_BA_states,
                                                                                                         caps)
