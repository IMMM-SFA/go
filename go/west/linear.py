...
def Sim_DRUp_DRDown(model, z, i):
    ...
    """
    if (pyo.value(model.HorizonDR_down[z, i]) != 0) and (pyo.value(model.HorizonDR_up[z, i]) != 0):
...
def DR_shifting(model, z):
    ...
    daily_DR_down_limit = sum(pyo.value(model.HorizonDR_down[z, i]) for i in model.hh_periods)
    daily_DR_up_limit = sum(pyo.value(model.HorizonDR_up[z, i]) for i in model.hh_periods)
